...
- **Caddy reverse proxy with automatic HTTPS** – a Caddy app listens on
  port 80 for ACME challenges and routes entire domains to the internal
  services rather than using path‑based routing.  Each service can have
  its own custom domain (for example `app1.example.com` and
  `app2.example.com`).  Caddy’s built‑in
  [ACME](https://caddyserver.com/docs/automatic-https) support obtains
  and renews certificates for these domains automatically.  You specify
  `APP1_DOMAIN` and `APP2_DOMAIN` along with a contact email
  (`LETS_ENCRYPT_EMAIL`) in your `.env` file.  The mounted Azure Files
  share stores the `Caddyfile` and certificate data, so certificates
  persist across revisions and restarts.  You must create DNS records
  pointing your custom domains at the external FQDN of the Caddy
  container app for ACME challenges to succeed.
...
| `caddy/Caddyfile` | Caddy configuration used by the reverse proxy.  It defines one site per custom domain and proxies to the appropriate internal service. |
| `caddy/caddy_app_template.yaml` | A YAML template used by the script to deploy or update the Caddy container app.  It contains placeholders for the environment ID, backend FQDNs, domain names and storage name. |
...
* You have set `APP1_DOMAIN`, `APP2_DOMAIN` and `LETS_ENCRYPT_EMAIL` in your `.env`
  file.  `APP1_DOMAIN` and `APP2_DOMAIN` are the fully qualified domains that clients
  use to reach each backend service via the Caddy reverse proxy.  Caddy uses
  these domains when requesting certificates from Let’s Encrypt.  `LETS_ENCRYPT_EMAIL`
  is the contact email registered with the certificate authority.
* You have created DNS records (A or CNAME) that point your `APP1_DOMAIN` and
  `APP2_DOMAIN` to the external FQDN of the Caddy container app.  You can look
  up the external FQDN by running `scripts/deploy.sh` once (it prints the
  FQDN on completion).  After updating your DNS records, re‑run the
  deployment script so that Caddy can obtain certificates for your domains.
...
Container Apps environment (either internal‑only or dual‑homed).  Two
internal services (`app1` and `app2`) run inside the environment.  A
Caddy reverse proxy is exposed externally and proxies traffic to the
internal services based on **custom domains** rather than URL paths.
Each service can have its own domain (for example `app1.example.com` and
`app2.example.com`), and Caddy uses these domains when issuing
certificates via Let’s Encrypt using the ACME protocol.  An Azure
storage account hosts the `Caddyfile` and stores Caddy’s certificate
data, which are mounted into the container via an Azure Files share.
Diagnostic logs are written to Azure Monitor via the configured
storage account.
