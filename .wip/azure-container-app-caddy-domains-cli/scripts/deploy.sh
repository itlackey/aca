...
sed \
  ...
  -e "s/{{APP1_DOMAIN}}/$APP1_DOMAIN/g" \
  -e "s/{{APP2_DOMAIN}}/$APP2_DOMAIN/g" \
...
echo "\nThe reverse proxy is configured for per‑app domains.  Ensure your DNS records for \n  $APP1_DOMAIN and $APP2_DOMAIN resolve to the Caddy FQDN above."
echo "\nTest the reverse proxy by navigating to your custom domains:"
echo "  http://$APP1_DOMAIN"
echo "  http://$APP2_DOMAIN"