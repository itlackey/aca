...
# Configure environment storage if not already configured
existing_storage=$(az containerapp env show --resource-group "$RESOURCE_GROUP" --name "$ACA_ENVIRONMENT" --query "properties.storage" -o tsv || true)
if ! echo "$existing_storage" | grep -q "$STORAGE_NAME"; then
  az containerapp env storage set \
    ...
    --output none
  echo "Configured environment storage '$STORAGE_NAME' for Azure Files."
else
  echo "Environment storage '$STORAGE_NAME' already configured."
fi
...
# Render the YAML by replacing placeholders with actual values
sed \
  -e "s/{{ENV_ID}}/$ENV_ID/g" \
