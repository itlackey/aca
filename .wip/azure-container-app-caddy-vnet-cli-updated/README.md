...
- **Caddy reverse proxy with automatic HTTPS** – a Caddy app listens on
  port 80 for ACME challenges and routes `/app1` and `/app2` paths to the
  internal services using environment variables for their fully qualified
  domain names.  Caddy’s built‑in [ACME](https://caddyserver.com/docs/automatic-https)
  support obtains and renews certificates for your public domain.  The
  domain and certificate contact email are provided in the `.env` file via
  `PUBLIC_DOMAIN` and `LETS_ENCRYPT_EMAIL`.  Caddy stores its
  configuration and certificate data on the mounted Azure Files share, so
  certificates persist across revisions and restarts.  A custom DNS record
  must point your domain at the Container Apps environment for ACME
...
* You have set `PUBLIC_DOMAIN` and `LETS_ENCRYPT_EMAIL` in your `.env`
  file.  `PUBLIC_DOMAIN` is the DNS name that clients use to reach the
  Caddy reverse proxy.  Caddy uses this domain when requesting a
  certificate from Let’s Encrypt.  `LETS_ENCRYPT_EMAIL` is the contact
  email registered with the certificate authority.
* You have created a DNS record (A or CNAME) that points your
  `PUBLIC_DOMAIN` to the external FQDN of the Caddy container app.  You
  can look up the FQDN by running `scripts/deploy.sh` once (it
  prints the FQDN on completion).  Update your DNS after the first
  deployment, then re‑run the deployment script to let Caddy obtain
  certificates.
...
internal services based on the URL path.  Caddy also automatically
obtains and renews TLS certificates for your public domain via
Let’s Encrypt using the ACME protocol.  An Azure storage account hosts
the `Caddyfile` and stores Caddy’s certificate data, which are mounted
into the container via an Azure Files share.  Diagnostic logs are
